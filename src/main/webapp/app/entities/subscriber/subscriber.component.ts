...
export class SubscriberComponent implements OnInit, OnDestroy {
  ...
  loadAll(): void {
    this.subscriberService
      ...
      .subscribe((res: HttpResponse<ISubscriber[]>) => this.paginateSubscribers(res.body, res.headers));
  }
  ...
  registerChangeInSubscribers(): void {
    this.eventSubscriber = this.eventManager.subscribe('subscriberListModification', () => this.reset());
  }
  ...
  protected paginateSubscribers(data: ISubscriber[] | null, headers: HttpHeaders): void {
    const headersLink = headers.get('link');
    this.links = this.parseLinks.parse(headersLink ? headersLink : '');
    if (data) {
      for (let i = 0; i < data.length; i++) {
        this.subscribers.push(data[i]);
      }
    }
  }
}
