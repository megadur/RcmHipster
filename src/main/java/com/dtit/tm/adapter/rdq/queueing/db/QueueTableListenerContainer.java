...
public class QueueTableListenerContainer {
   ...
   */
  public QueueTableListenerContainer(
    TransactionTemplate newTxTemplate,
    JdbcTemplate jdbcTemplate,
    String tableName,
    int poolSize,
    long pollingRate,
    BiFunction<String, String, Boolean> serviceFunction,
      ...
      newTxTemplate.execute(
        status -> {
          ...
          if (dispatchKey != null) {
            lstCount =
              jdbcTemplate.query(
                "SELECT count(id) from " + tableName + " where dispatch_key=?",
                new RowMapperResultSetExtractor<>((rs, rowNum) -> rs.getLong(1), 1),
                dispatchKey
              );
          } else {
            lstCount =
              jdbcTemplate.query("SELECT count(id) from " + tableName, new RowMapperResultSetExtractor<>((rs, rowNum) -> rs.getLong(1), 1));
          }
      ...
      newTxTemplate.execute(
        new TransactionCallbackWithoutResult() {
          ...
          protected void doInTransactionWithoutResult(TransactionStatus status) {
            List<Message> lstMessage = jdbcTemplate.query(
              pollerQueryString,
